...
<div class="tableContainer" id="div-report"  align="center">
  ...
  <?php 
    ...
    echo $this->Form->button(__('Mark Attendance'),['class' => 'mbtn','disabled'=>'disabled']);
    echo $this->Form->end();

  ?>
<br /><br /><br /><br /><br />
<script>
    $(document).on('click', '.chkView', function() {
        
       
        if($(this).is(':checked'))
        {
        $(this).closest('tr').find('.first_half, .second_half,.shift_id,.att_date,.mbtn').prop('disabled', false);
		
		$('.mbtn').prop('disabled', false);
		
		$('.first_half, .second_half').prop('required',true);
        
        }
        else{
            
           $(this).closest('tr').find('.first_half, .second_half,.shift_id,.att_date,.mbtn').prop('disabled', true); 
		  
		   
		   $('.first_half, .second_half').prop('required',false);
        }
